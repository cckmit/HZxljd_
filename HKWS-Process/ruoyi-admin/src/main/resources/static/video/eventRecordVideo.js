...
$(function() {
	...
	// 监听resize事件，使插件窗口尺寸跟随DIV窗口变化
	 $(window).resize(function () { 
	    if (oWebControl != null) {
	        oWebControl.JS_Resize(345, 250);//345, 250
	        setWndCover();
	    }
	});
    ...
    oWebControl = new WebControl({
		...
		cbConnectSuccess: function () {                     // 创建WebControl实例成功											
			oWebControl.JS_StartService("window", {         // WebControl实例创建成功后需要启动服务
				...
				oWebControl.JS_CreateWnd("playWnd", 345, 250).then(function () { //JS_CreateWnd创建视频播放窗口，宽高可设定
					init();  // 创建播放实例成功后初始化
				});
			}, function () { // 启动插件服务失败
			});
...
function init()
{
    getPubKey(function () {
        ...
        oWebControl.JS_RequestInterface({
			...
			oWebControl.JS_Resize(345, 250);  // 初始化后resize一次，规避firefox下首次显示窗口后插件窗口未与DIV窗口重合问题
            startPlayback();
        });
    });
}
