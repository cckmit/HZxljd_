        ...
        tree: {
        	...
        	init: function(options) {
        	    ...
        	    $.get(options.url, function(data) {
        			...
        			if (options.isCustom == '1'){
						$.tree.zTreeOnAsyncSuccess(tree,options.checkedTreeValue); //自定义回显已选中的节点
					}
        		});
        	},
			zTreeOnAsyncSuccess:function(treeObj,value){ //回显已选中的节点
				$.each(value,function (i,item) {
					treeObj.checkNode(treeObj.getNodeByParam("id", item.regionId),true)
				})
			},
        	// 搜索节点
        	searchNode: function() {
            ...
            // 获取末级被勾选集合
            getCheckedLastNodes: function() {
        		debugger;
        		var column1 = "isParent";
        		var column2 = "id";
                var nodes = $._tree.getCheckedNodes(true);
                return $.map(nodes, function (row) {
                	if(!row[column1]){
